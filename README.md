...
Проект представляет собой консольное приложение и библиотеку на C# для широкого спектра геодезических расчетов: преобразования координат между различными системами (датумами), проекциями, форматами, а также для решения прикладных задач триангуляции (определение координат наблюдателя и цели по данным с двух опорных объектов). В основе лежат алгоритмы, соответствующие стандартам (в частности, ГОСТ 32453-2017).
...
### 6. Прикладные задачи (триангуляция)
В классе `TargetLocator` (часть `ParserGMS.cs`) реализованы методы для решения обратной засечки:
*   **`GetTargetCoordinates`** — вычисление координат **наблюдателя** и **наземной цели** по данным с двух опорных точек (наклонные дальности, горизонтальные углы и углы места).
*   **`GetTargetCoordinatesAlternative`** — альтернативный метод на основе векторного вращения.
*   **`FindCircleIntersection`** — нахождение точек пересечения двух окружностей на плоскости (вспомогательная геометрическая задача).
